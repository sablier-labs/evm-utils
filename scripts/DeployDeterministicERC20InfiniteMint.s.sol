...
contract DeployDeterministicERC20InfiniteMint is BaseScript {
    function run() public broadcast returns (ERC20InfiniteMint token) {
        bytes32 salt = keccak256(abi.encodePacked("This is an infinite minting token"));
        token = new ERC20InfiniteMint{ salt: salt }();
    }
}
