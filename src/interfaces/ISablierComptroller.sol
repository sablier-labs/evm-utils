...
interface ISablierComptroller is IRoleAdminable {
    ...
    event DisableCustomFeeUSD(Protocol indexed protocol, address msgSender, address indexed user);
    ...
    event SetCustomFeeUSD(Protocol indexed protocol, address msgSender, address indexed user, uint256 customFeeUSD);

    /// @notice Emitted when the admin or the fee manager sets a new minimum USD fee.
    event SetMinFeeUSD(Protocol indexed protocol, address msgSender, uint256 previousMinFeeUSD, uint256 newMinFeeUSD);
